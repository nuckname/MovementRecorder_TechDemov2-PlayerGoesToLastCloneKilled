    ...
    public class SurfCharacter : MonoBehaviour, ISurfControllable
    {
        ...
        public void ApplyExternalForce(Vector3 force)
        {
            if (rb != null)
            {
                ...
                StartCoroutine(ReenableKinematicAfterTime(0.1f)); // Re-enable kinematic after a short delay
            }
        }

        private IEnumerator ReenableKinematicAfterTime(float time)
        {
        ...
        //Talking to Teleport Player.
        //Created in, in-game time
        //player rotation
        ...
        private void Start () {
            ...
            _colliderObject.transform.rotation = Quaternion.Euler(0,47,0);
        ...
        private void Update () {
            ...
            //removing this make it so that _colliderObject doesnt update. 
            //_colliderObject.transform.rotation = Quaternion.identity;
            //_colliderObject.transform.rotation is accessing wrong thing .
            // Desired rotation angles
            Vector3 newRotationEulerAngles = new Vector3(0, 57, 0);

            //  Convert the Euler angles to a quaternion and apply it to the transform's rotation
            _colliderObject.transform.rotation = Quaternion.Euler(newRotationEulerAngles);

            // If you need to add or modify the rotation based on the current rotation
            _colliderObject.transform.rotation = Quaternion.Euler(_colliderObject.transform.rotation.eulerAngles + newRotationEulerAngles);

            //print(_colliderObject.transform.rotation.eulerAngles);
            //UpdateTestBinds ();
            UpdateMoveData ();
        ...
        //On play creates _colliderObject
        //This is a trigger. 
        private void OnTriggerEnter (Collider other) 
        {
            if (other.gameObject.CompareTag("FinishLine"))
            {
                //playerCollision.PlayerCol(other);
            }
        ...
        {
            if (other.gameObject.CompareTag("TronBullet"))
            {
                print("exploded");
            }
        }
